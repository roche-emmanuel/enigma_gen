...
Returns:
	Array of files. Raise an error if file names or pattern are not strings.
]]
function dir.filter(files,pattern)
    ...
    for i,f in ipairs(files) do
        if f:find(mask) then table.insert(res,f) end
    end
...
Returns:
	An array of files. Raise an error if dir or mask are not strings.
]]
...
Returns:
	Array containing all the files found recursively starting at *start_path* and filtered by *pattern*.
]]
